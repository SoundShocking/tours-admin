...
import { useDeleteService, usePagination, useServices } from '@/hooks'
...
export const ServicesPage: FC = () => {
  ...
  const { isFetching, services } = useServices(page, perPage, '', order, by)
  const { deleteServiceMutateAsync, isDeleting } = useDeleteService()

  const onEdit = (id: number) => {
    navigate(routes.services.edit(id))
  }
  ...
  const onDelete = (id: number) => {
    confirm({
      okType: 'danger',
      onOk() {
        return deleteServiceMutateAsync(id, {
          onSuccess: () => {
  ...
  const columns: ColumnsType<IService> = [
    ...
    {
      dataIndex: 'name',
      render: (_, record) => <Link to={`${routes.services.edit(record.id)}`}>{record.name}</Link>,
      sorter: true,
      title: 'Name',
      width: 300,
    },
