import type { ItemType } from 'antd/es/breadcrumb/Breadcrumb'

import { FC, useEffect } from 'react'
import { Link, useParams } from 'react-router-dom'

import { ContentLanguageSwitcher } from '@/components'
import { NOTIFICATION_PLACEMENT } from '@/constants'
import { usePromotion, useUpdatePromotion } from '@/hooks'
import { routes } from '@/routes'
import {
  Breadcrumb,
  Button,
  Card,
  DatePicker,
  Flex,
  Form,
  Input,
  InputNumber,
  Select,
  Spin,
  notification,
} from 'antd'
import dayjs from 'dayjs'
import { FaFloppyDisk } from 'react-icons/fa6'
import { useDocumentTitle } from 'usehooks-ts'

export const PromotionEditPage: FC = () => {
  useDocumentTitle('Edit Promotion')

  const breadcrumbs: ItemType[] = [
    {
      title: <Link to={routes.promotions.index}>Promotions</Link>,
    },
    {
      title: 'Edit Promotion',
    },
  ]

  const [form] = Form.useForm()

  const { id } = useParams<{ id: string }>()
  const { isFetching, promotion } = usePromotion(id!)
  const { isUpdating, updatePromotionMutate } = useUpdatePromotion(id!)

  useEffect(() => {
    if (promotion) {
      const validTo = dayjs(promotion.valid_to)

      form.setFieldsValue({
        name: promotion.name,
        valid_to: validTo.isValid() ? validTo : null,
        value: promotion.value,
        value_type: promotion.value_type,
      })
    }
  }, [promotion, form])

  const onFinish = (values: any) => {
    const { valid_to, ...rest } = values
    const validTo = dayjs(valid_to)

    const form = {
      ...rest,
      api_id: promotion?.api_id,
      pax_restriction: promotion?.pax_restriction,
      valid_to: validTo.isValid() ? validTo.format('YYYY-MM-DD') : null,
    }

    updatePromotionMutate(form, {
      onSuccess: () => {
        notification.success({
          message: 'The promotion was updated successfully',
          placement: NOTIFICATION_PLACEMENT,
        })
      },
    })
  }

  return (
    <Flex gap={'large'} vertical>
      <Flex align={'center'} justify={'space-between'}>
        <Breadcrumb items={breadcrumbs} />

        <ContentLanguageSwitcher />
      </Flex>

      <Card className={'resource-edit-form'}>
        <Spin spinning={isFetching}>
          <Form form={form} layout={'vertical'} onFinish={onFinish}>
            <Form.Item label={'Name'} name={'name'} rules={[{ required: true }]}>
              <Input />
            </Form.Item>

            <Form.Item label={'Value Type'} name={'value_type'} rules={[{ required: true }]}>
              <Select
                options={[
                  { label: 'Percent', value: 'percent' },
                  { label: 'Fix', value: 'fix' },
                ]}
              />
            </Form.Item>

            <Form.Item label={'Value'} name={'value'} rules={[{ required: true }]}>
              <InputNumber />
            </Form.Item>

            <Form.Item label={'Valid To'} name={'valid_to'} rules={[{ required: true }]}>
              <DatePicker format={{ format: 'YYYY-MM-DD' }} />
            </Form.Item>

            <Button
              htmlType={'submit'}
              icon={<FaFloppyDisk />}
              loading={isUpdating}
              type={'primary'}
            >
              Save
            </Button>
          </Form>
        </Spin>
      </Card>
    </Flex>
  )
}
